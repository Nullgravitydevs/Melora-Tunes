...
export function MoodDetailScreen({ mood, colors, onBack, onOpenPlaylist, languageContext = 'english,hindi' }: MoodDetailScreenProps) {
    ...
    // Fetch mood content
    useEffect(() => {
        // Reset radio state on mood change
        setRadioStarted(false); // FIX 4: Reset state

        const fetchMoodContent = async () => {
            setLoading(true);
            try {
                // Fetch playlists for this mood
                const moodPlaylists = await searchPlaylists(`${mood.name} playlist`, 1, 10, langContext);
                setPlaylists(moodPlaylists.slice(0, 8));

                // Fetch songs for this mood
                const moodSongs = await searchSongs(`${mood.name} songs`, 1, 20, langContext);
                setSongs(moodSongs);
            } catch (e) {
        ...
        fetchMoodContent();
    }, [mood.id, mood.name, langContext]); // FIX 2: Dependencies
    ...
    const handleStartRadio = () => {
        ...
        // Shuffle songs for variety (Limit seed)
        const shuffled = [...songs]
            .sort(() => Math.random() - 0.5)
            .slice(0, 25)
            .map(s => ensurePlayableTrack(s)); // FIX: Safety

        // Create mix with mood name
        playInstantMix({
            id: `mood-radio-${mood.id}-${Date.now()}`, // FIX 3: Unique ID
    ...
    // Shuffle All - plays shuffled mix WITHOUT radio mode
    const handleShuffleAll = () => {
        ...
        playInstantMix({
            id: `mood-${mood.id}-shuffle-${Date.now()}`,
            title: `${mood.name} Shuffle`,
            color: 'pink',
            songs: shuffled,
            currentSongIndex: 0
        });
    ...
    const handlePlaySong = (song: any, index: number) => {
        ...
        playInstantMix({
            id: `mood-${mood.id}-from-${index}-${Date.now()}`, // FIX 3: Unique ID
            title: `${mood.name} Mix`,
            color: 'pink',
            songs: queuedSongs.map(s => ensurePlayableTrack(s)), // FIX 1: Safety
            currentSongIndex: 0
        });
    };
        ...
        <div className="flex-1 flex flex-col overflow-y-auto w-full h-full bg-black">
            ...
            >
                ...
                <motion.button
                    ...
                    disabled={loading || songs.length === 0 || radioStarted} // FIX 5: UX Polish
